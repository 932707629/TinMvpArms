...
//        ALog.e("最终结果",dataFormat(data));
    ...
    private  final static String regex="\\[(\\d+|\\d.\\d+|0\\.\\d+)秒\\]";
    private String dataFormat(String data) {
        for (int i = 50; i >0; i--) {
            List<String> seconds=getSeconds(data,i);
            if (seconds.size()>0){
                for (String s:seconds) {
                    String oldTime = s.replace("[", "-").replace("秒]", "-");
                    String[] oldTimes=oldTime.split("-");
                    int time=0;
                    for (String ot:oldTimes) {
                        if (ot!=null&&!ot.isEmpty()){
                            time=time+((int)(Float.parseFloat(ot)*1000));
                        }
                    }
                    String newTime = "<break time=" + time + "ms/>";
                    data = data.replace(s, newTime);
                }
            }
        }
        return "<speak>" + data + "</speak>";
    }
     ...
     * @return
     */
    public List<String> getSeconds(String s,@IntRange(from = 0) int range) {
        StringBuilder sRegex=new StringBuilder();
        for (int i = 0; i < range; i++) {
            sRegex.append(regex);
        }
        List<String> strs = new ArrayList<>();
        Pattern p = Pattern.compile(sRegex.toString());
